  ...
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (FundOrigin.openapiRequiredFields.isEmpty()) {
          ...
          throw new IllegalArgumentException(String.format("The required field(s) %s in FundOrigin is not found in the empty JSON string", FundOrigin.openapiRequiredFields));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!FundOrigin.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `FundOrigin` properties. JSON: %s", entry.getKey(), jsonObj));
        }
      }
