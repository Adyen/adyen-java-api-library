  ...
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (PermitResult.openapiRequiredFields.isEmpty()) {
          ...
          throw new IllegalArgumentException(String.format("The required field(s) %s in PermitResult is not found in the empty JSON string", PermitResult.openapiRequiredFields));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!PermitResult.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PermitResult` properties. JSON: %s", entry.getKey(), jsonObj));
        }
      }
