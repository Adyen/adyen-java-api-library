  ...
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (Expiry.openapiRequiredFields.isEmpty()) {
          ...
          throw new IllegalArgumentException(String.format("The required field(s) %s in Expiry is not found in the empty JSON string", Expiry.openapiRequiredFields));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!Expiry.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `Expiry` properties. JSON: %s", entry.getKey(), jsonObj));
        }
      }
