  ...
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (MerchantLinks.openapiRequiredFields.isEmpty()) {
          ...
          throw new IllegalArgumentException(String.format("The required field(s) %s in MerchantLinks is not found in the empty JSON string", MerchantLinks.openapiRequiredFields));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!MerchantLinks.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `MerchantLinks` properties. JSON: %s", entry.getKey(), jsonObj));
        }
      }
