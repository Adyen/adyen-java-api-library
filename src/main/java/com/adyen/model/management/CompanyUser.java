  ...
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (CompanyUser.openapiRequiredFields.isEmpty()) {
          ...
          throw new IllegalArgumentException(String.format("The required field(s) %s in CompanyUser is not found in the empty JSON string", CompanyUser.openapiRequiredFields));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!CompanyUser.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CompanyUser` properties. JSON: %s", entry.getKey(), jsonObj));
        }
      }
