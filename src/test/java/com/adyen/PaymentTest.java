...
public class PaymentTest extends BaseTest {
  ...
  public void TestAuthoriseSuccessMocked() throws Exception {
    ...
    FraudCheckResult fraudCheckResult =
        paymentResult.getFraudResult().getResults().get(0).getFraudCheckResult();
  ...
  public void TestRetrieve3ds2ResultSuccessMocked() throws Exception {
    ...
    ThreeDS2ResultResponse threeDS2ResultResponse =
        payment.retrieve3ds2Result(threeDS2ResultRequest);

    assertEquals(threeDS2ResultResponse.getThreeDS2Result().getTransStatus(), "Y");
    assertEquals(
        threeDS2ResultResponse.getThreeDS2Result().getAuthenticationValue(),
        "3q2+78r+ur7erb7vyv66vv8deha8=");
    assertEquals(threeDS2ResultResponse.getThreeDS2Result().getEci(), "07");
    assertEquals(
        threeDS2ResultResponse.getThreeDS2Result().getThreeDSServerTransID(),
        "73aab3ce-eb39-49e8-8e9b-46fb77a472f1");
  }
  ...
  public void TestAuthoriseCSEErrorExpiredMocked() throws Exception {
    ...
    assertEquals(
        "DECLINED Expiry Incorrect",
        paymentResult.getAdditionalData().get(AdditionalData.REFUSAL_REASON_RAW));
  }
  ...
  public void TestError401Mocked() throws Exception {
    ...
    HTTPClientException httpClientException =
        new HTTPClientException(401, "An error occured", new HashMap<>(), null);
    ...
    when(adyenHttpClient.request(
            anyString(), anyString(), any(Config.class), anyBoolean(), isNull(), any(), isNull()))
        .thenThrow(httpClientException);
  ...
  public void TestOpenInvoice() throws Exception {
    ...
    PaymentResult paymentResult = payment.authorise(paymentRequest);
    assertEquals(
        "2374421290", paymentResult.getAdditionalData().get("additionalData.acquirerReference"));
  ...
  public void TestBoletoSuccess() throws Exception {
    ...
    PaymentRequest paymentRequest =
        createBasePaymentRequest(new PaymentRequest())
            .reference("123456")
    ...
    assertEquals(
        "34191.75801 12021.372227 21111.100000 8 71670000001000",
        additionalData.get(BOLETO_BARCODE_REFERENCE));
    assertEquals(
    ...
    assertEquals(
        "2017-05-22", fmt.format(DateUtil.parseYmdDate(additionalData.get(BOLETO_DUE_DATE))));
    assertEquals(
    ...
    assertEquals(
        "2017-06-06",
        fmt.format(DateUtil.parseYmdDate(additionalData.get(BOLETO_EXPIRATION_DATE))));
  ...
  public void TestByteArrayDeserialization() throws Exception {
    ...
    // Act
    byte[] actualDeserializedBytes =
        JSON.getMapper().readValue(serializedBytesWithQuotes, byte[].class);
  ...
  public void TestByteArrayToJSONString() throws Exception {
    ...
    verify(http)
        .request(anyString(), contains(expected), any(), eq(false), isNull(), any(), isNull());
  }
