...
public class BaseTest {
  protected final ApplicationInfo applicationInfo =
      new ApplicationInfo().adyenLibrary(new CommonField().name(LIB_NAME).version(LIB_VERSION));
  protected static final Gson PRETTY_PRINT_GSON =
      new GsonBuilder()
  ...
  protected Client createMockClientFromResponse(String response) {
    ...
    try {
      when(adyenHttpClient.request(
              anyString(), anyString(), any(Config.class), anyBoolean(), isNull()))
          .thenReturn(response);
      when(adyenHttpClient.request(
  ...
  protected PaymentRequest createBasePaymentRequest(PaymentRequest abstractPaymentRequest) {
    return abstractPaymentRequest
        ...
        .browserInfo(new BrowserInfo().userAgent(USER_AGENT).acceptHeader("*/*"))
        .shopperIP("1.2.3.4");
  }
  ...
  protected PaymentRequest createFullCardPaymentRequest() {
    ...
    card.setCvc("737");
    return createBasePaymentRequest(new PaymentRequest())
        .reference("123456")
  ...
  protected PaymentRequest createOpenInvoicePaymentRequest() {
    ...
    PaymentRequest paymentRequest =
        createBasePaymentRequest(new PaymentRequest()).reference("123456");
  ...
  protected void setInvoiceLines(PaymentRequest paymentRequest, List<InvoiceLine> invoiceLines) {
    Map<String, String> additionalData =
        paymentRequest.getAdditionalData() != null
    ...
    for (InvoiceLine invoiceLine : invoiceLines) {
      ...
      additionalData.put(
          lineNumber + ".itemVatPercentage", invoiceLine.getItemVatPercentage().toString());
      additionalData.put(
          lineNumber + ".numberOfItems", Integer.toString(invoiceLine.getNumberOfItems()));
  ...
  protected PaymentRequest3d create3DPaymentRequest() {
    return createBasePaymentRequest(new PaymentRequest3d())
        .md("mdString")
  ...
  protected TerminalAPIRequest createTerminalAPIPaymentRequest()
